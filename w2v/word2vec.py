...
from sampler import sample_word2vec,random_sampler
...
class Word2Vec(utils.SaveLoad):
    ...
    """
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5, seed=1, workers=1, min_alpha=0.0001, sampler=None,context=False,p=[0.8,0.2]):
        """
        ...
        """
        self.vocab = {}  # mapping from a word (string) to a Vocab objects
        ...
        def worker_train():
            ...
            while True:
                ...
                if self.sampler:
                    ...
                    if self.context:
                        job_words = sum(train_sentence_sampler(self, self.sampler(sentence,self.window,self.p), len(filter(None, sentence)), alpha, work)
                                    for sentence in job)
                    else:
...
if __name__ == "__main__":
    ...
    model = Word2Vec(Text8Corpus(infile), size=200, min_count=5, workers=1, sampler=random_sampler)
